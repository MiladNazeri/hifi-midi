var URL = "https://s3-us-west-1.amazonaws.com/hifi-planets/";

Script.include(Script.resolvePath('tools/vector.js'));
/*
  planetConfig
  DAMPING = 0.0
  EARTH_SIZE = 7.0;
  referenceRadius = 15.0;
  referenceDiameter = 0.6;
  referencePeriod = 3.0;
  LARGE_BODY_MASS = 250.0;
  VELOCITY_OFFSET_Y = -0.3;
  VELOCITY_OFFSET_Z = 0.9;
  MOON_RADIUS = 0.387;
  MOON_SCALE = 0.383;
*/
var planetConfig = Script.require(Script.resolvePath('Tablet/planetDefault.json?v=' + Date.now()));

var APP_URL = Script.resolvePath('Tablet/planetUI.html');
...
function onWebEventReceived(event) {
    ...
    switch (htmlEvent.type){
        ...
        // case 'reset':
            ...
            // planetConfig = makeCopy(backupFish);
            // fishLoaded = false;
            // cleanupFish();
            // break;
        default:
...
CreateSimulation = function() {
    ...
    // var DAMPING = this.DAMPING = planetConfig.DAMPING.val;
    var LIFETIME = this.LIFETIME = 6000;
    // var BASE_URL = this.BASE_URL = "https://s3-us-west-1.amazonaws.com/hifi-planets/";
    ...
    // var EARTH_SIZE = this.EARTH_SIZE = planetConfig.EARTH_SIZE.val;
    ...
    var SMALL_BODY_MASS = this.SMALL_BODY_MASS = planetConfig.LARGE_BODY_MASS.val * 0.000000333;
    function getGravity(){
    ...
    var Planet = function(name, trailColor, radiusScale, sizeScale) {
        ...
        this.radius = radiusScale * planetConfig.referenceRadius.val;
        ...
        this.period = (2.0 * Math.PI) * Math.sqrt(Math.pow(this.radius, 3.0) / (getGravity() * planetConfig.LARGE_BODY_MASS.val));

        this.initialVelocity = Math.sqrt((getGravity() * planetConfig.LARGE_BODY_MASS.val) / this.radius);
        ...
        this.dimensions = sizeScale * planetConfig.referenceDiameter.val;
        ...
        this.computeAcceleration = function() {
            var acc = -(getGravity() * planetConfig.LARGE_BODY_MASS.val) * Math.pow(this.radius, (-2.0));
            return acc;
        };
    ...
    this.initPlanets = function() {
        planets.push(new Planet("moon", MOON_LINE_COLOR, planetConfig.MOON_SCALE.val, planetConfig.MOON_RADIUS.val));
    ...
    var scriptEnding = this.scriptEnding = function() {
        print("button:=======", button);
        Entities.deleteEntity(theEarth);
        for (var i = 0; i < planets.length; ++i) {
